...
app.post("/random-instagram-comment-picker", async (req, res) => {
  ...
  if (!token)
    return renderTemplate(res, req, "index.ejs", {
      alert: "A token must be specified.",
      payload: null,
    });
  if (!url)
    return renderTemplate(res, req, "index.ejs", {
      alert: "An url must be specified.",
      payload: null,
    });
  if (!amount)
    return renderTemplate(res, req, "index.ejs", {
      alert: "An amount must be specified.",
      payload: null,
    });
  ...
  try {
    igMedia = await fetch(
      `https://api.instagram.com/oembed/?callback=&url=${url}`
    ).then((r) => r.json());
  } catch (e) {
    return renderTemplate(res, req, "index.ejs", {
  ...
  if (amount > comments.length)
    payload["winners"] = shuffle(comments).map((w) => {
  ...
  for (var i = 0; i <= amount - 1; i++) {
    payload["winners"].push({
      username: comments[i].from.username,
...
app.listen(8181, () => {
  console.log("Server is running at port 8181. Visit http://localhost:8181/");
});
